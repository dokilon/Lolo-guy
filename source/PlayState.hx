	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'LoloIsGay': //Week Lolo
		        ...
		        var loloft:FlxSprite = new FlxSprite( -400, 700); //x, y + left - right y: +up -down
				loloft.frames = Paths.getSparrowAtlas('bobfront');
                loloft.animation.addByPrefix('bob', 'bg', 16, true);
				loloft.setGraphicSize(Std.int(lolob.width * 0.6));
				loloft.updateHitbox();
				loloft.animation.play('bob');
				loloft.antialiasing = true;
				add(loloft);
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'weekl_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss':
